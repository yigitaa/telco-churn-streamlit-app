...
def get_user_inputs(df, input_cols, internet_service_cols, phone_service_cols, main_internet_col, main_phone_col, special_col):
    """
    ...
    if main_internet_col:
        user_input[main_internet_col] = cols[0].selectbox(main_internet_col, options=df[main_internet_col].unique(), key=main_internet_col)
    if main_phone_col:
        user_input[main_phone_col] = cols[1].selectbox(main_phone_col, options=df[main_phone_col].unique(), key=main_phone_col)
    if user_input.get(main_internet_col) is not None and user_input.get(main_phone_col) is not None:
        cols = st.columns(3)
        user_input[special_col] = cols[0].selectbox(special_col, options=["No", "Yes"], key=special_col)
        col_idx = 1
        for col in input_cols:
            ...
            if col == "tenure":
                user_input[col] = int(current_col.number_input(col, value=int(df[col].mean()), key=col, disabled=disabled, step=1, format="%d"))
            elif col == "MonthlyCharges":
                user_input[col] = float(current_col.number_input(col, value=float(df[col].mean()), key=col, disabled=disabled, format="%.2f"))
            elif df[col].dtype == 'object' or str(df[col].dtype) == 'category':
                user_input[col] = current_col.selectbox(col, options=df[col].unique(), key=col, index=list(df[col].unique()).index(default_val), disabled=disabled)
            else:
                user_input[col] = current_col.number_input(col, value=float(df[col].mean()), key=col, disabled=disabled)
            col_idx += 1
